...
function registrar_taxonomias_filmes() {
    ...
    foreach ($taxonomias as $slug => $nome) {
        ...
        // Registro seguro da taxonomia
        if (!taxonomy_exists($slug)) {
            register_taxonomy($slug, 'filmes', $args);
        }
    }
}
...
function registrar_cpt_filmes() {
    $args = [
        ...
        'show_in_rest' => true,
        'taxonomies' => ['distribuidoras', 'paises', 'generos', 'classificacoes', 'tecnologias', 'post_tag'],
    ];
...
/**
 * Função auxiliar para obter IDs de termos com tratamento robusto
 */
...
/**
 * Schema para os dados do filme com tratamento seguro
 */
function filme_scheme($post) {
    ...
    // Processa fotos com verificação
    ...
    // Processa taxonomias com verificação
...
/**
 * Função auxiliar para processar termos de taxonomias com tratamento seguro
 */
...
/**
 * Endpoint POST para criar filmes com tratamento seguro
 */
function api_filmes_post($request) {
    ...
    // Processa arrays com verificação
    ...
    // Prepara dados para o CFS com tratamento seguro
    $field_data = [
